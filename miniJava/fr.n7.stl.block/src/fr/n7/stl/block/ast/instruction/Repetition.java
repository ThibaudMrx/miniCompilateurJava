...
public class Repetition implements Instruction {
	...
	public boolean collect(HierarchicalScope<Declaration> _scope) {
		return this.condition.collect(_scope) && this.body.collect(_scope);
	}
	...
	public Fragment getCode(TAMFactory _factory) {
		int num_rep = _factory.createLabelNumber();
		Fragment _fragment = this.condition.getCode(_factory);
		_fragment.add(_factory.createJumpIf("fin_rep_"+ num_rep, 0));
		
		_fragment.append(this.body.getCode(_factory));
		_fragment.add(_factory.createJump("debut_rep_"+ num_rep));
		
		_fragment.addPrefix("debut_rep_" + num_rep);
		_fragment.addSuffix("fin_rep_" + num_rep);
		return _fragment;
