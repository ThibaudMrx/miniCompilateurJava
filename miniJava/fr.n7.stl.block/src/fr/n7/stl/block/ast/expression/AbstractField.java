...
public abstract class AbstractField implements Expression {
	...
	public boolean resolve(HierarchicalScope<Declaration> _scope) {
		...
		Type recordt = this.record.getType();
		if (recordt instanceof NamedType) {
			recordt = ((NamedType) recordt).getType();
		}
		if (recordt instanceof RecordType) {
			RecordType recordcast = (RecordType) recordt;
			this.field = recordcast.get(this.name);
		} else {
			throw new BlockSemanticsError("objet pas un enregistrement");
		}
		return ok;
	}
