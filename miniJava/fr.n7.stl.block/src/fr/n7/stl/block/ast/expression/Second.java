...
public class Second implements Expression {
	...
	public Type getType() {
		if (this.target.getType() instanceof CoupleType) {
			...
			throw new BlockSemanticsError("erreur getType Second");
		}
	}
	...
	public Fragment getCode(TAMFactory _factory) {
		...
		_fragment.add(_factory.createPop(this.getType().length(), this.target.getType().length() - this.getType().length()));
		return _fragment;
