...
public class VariableAssignment extends AbstractIdentifier implements AssignableExpression {
	...
	@Override
	public Type getType() {
		if (this.declaration.checkType()) {
			return this.declaration.getType();
		} else {
			throw new BlockSemanticsError("Variable Assignment getType Error");
		}
	...
	public Fragment getCode(TAMFactory _factory) {
		...
		if (this.getType() instanceof PointerType) {
			...
			_fragment.add(_factory.createStore(this.declaration.getRegister(), this.declaration.getOffset(), this.declaration.getType().length()));
		}
		return _fragment;
