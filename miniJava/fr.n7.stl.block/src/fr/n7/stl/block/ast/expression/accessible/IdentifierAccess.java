		...
		if  (this.expression == null) {
			if (((HierarchicalScope<Declaration>)_scope).knows(this.name)) {
				...
				if (_declaration instanceof VariableDeclaration) {
					...
					Type declarationType = _declaration.getType();
					...
					if (_declaration instanceof ConstantDeclaration) {
						...
						return true;
					} else if (declarationType instanceof EnumerationType) {
						//cast en enumerationtype
						EnumerationType enumDeclType = (EnumerationType) declarationType;
						String labelName = _declaration.getName();
						int declIndex = enumDeclType.getIndex(labelName);
						IntegerValue IntegerIndex = new IntegerValue(String.valueOf(declIndex));
						//on utilise une ConstantAccess pour récupérer la valeur de l'énumération
						this.expression = new ConstantAccess(new ConstantDeclaration(_declaration.getName(), _declaration.getType(), IntegerIndex));
						return true;
					} else {
