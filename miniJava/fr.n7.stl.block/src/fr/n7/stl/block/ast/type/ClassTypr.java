 ...
 *
 */
public class ClassTypr implements Type, Declaration, Scope<FieldDeclaration> {
	 ...
	 */
	public ClassTypr(String _name, Iterable<FieldDeclaration> _fields) {
		this.name = _name;
	...
	public boolean equalsTo(Type _other) {
		...
		if (_other instanceof RecordType) {
			...
			if (other.length() == this.length()) {
				for (FieldDeclaration f : this.fields) {
					FieldDeclaration champ = other.get(f.getName());
					if (champ != null) {
						ok = ok && f.getType().equalsTo(champ.getType());
					} else {
						ok = false;
					}
				}
		...
		return ok; */
		return (_other).equalsTo(this.getType());
	}
		...
		if (_other instanceof RecordType) {
			...
			if (other.length() == this.length()) {
				for (FieldDeclaration f : this.fields) {
					FieldDeclaration champ = other.get(f.getName());
					if (champ != null) {
						ok = ok && f.getType().compatibleWith(champ.getType());
					} else {
						ok = false;
					}
				}
	...
	public Type merge(Type _other) {
		if (_other instanceof ClassTypr) {
			ClassTypr other = (ClassTypr) _other;
			for (FieldDeclaration f : other.fields) {
				if (!this.contains(f.getName())) {
					this.add(f);
				}
			}
			return this;
		} else {
			throw new BlockSemanticsError("types non fusionnables");
		}
		//throw new SemanticsUndefinedException( "compatibleWith is undefined in RecordType.");
	}
	...
	public void AllocateMemory(Register _register, int _offset) {
		...
		for (FieldDeclaration fd : this.fields) {
			fd.setOffset(_offset+i);
			fd.setRegister(_register);
			i = i + fd.getType().length();
		}
	}
}
